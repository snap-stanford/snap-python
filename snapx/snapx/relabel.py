...
def relabel_nodes(G, mapping, copy=True):
    ...
    >>> import string
    >>> G = sx.path_graph(26)  # nodes are integers 0 through 25
    >>> sorted(G)[:3]
    [0, 1, 2]
    >>> mapping = dict(zip(G, string.ascii_lowercase))
    >>> G = sx.relabel_nodes(G, mapping) # nodes are characters a through z
    >>> sorted(G)[:3]
    ['a', 'b', 'c']
    >>> mapping = dict(zip(G, range(1, 27)))
    >>> G = sx.relabel_nodes(G, mapping)  # nodes are integers 1 through 26
    >>> sorted(G)[:3]
    [1, 2, 3]
    ...
    >>> G = sx.path_graph(3)  # nodes 0-1-2
    >>> mapping = {0: 'a', 1: 'b'} # 0->'a' and 1->'b'
    >>> G = sx.relabel_nodes(G, mapping, copy=False)
    >>> sorted(G, key=str)
    [2, 'a', 'b']
    ...
    >>> G = sx.path_graph(3)
    >>> H = sx.relabel_nodes(G, lambda x: x ** 2)
    >>> list(H)
    [0, 1, 4]
...
def _relabel_inplace(G, mapping):
    ...
    if len(old_labels & new_labels) > 0:
        ...
        D = sx.DiGraph(list(mapping.items()))
        D.remove_edges_from(sx.selfloop_edges(D))
        try:
            nodes = reversed(list(sx.topological_sort(D)))
        except sx.NetworkXUnfeasible as e:
            raise sx.NetworkXUnfeasible(
