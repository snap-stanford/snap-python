...
Test cases are copied from NetworkX.
"""
...
class BaseGraphTester:
    ...
    def test_nbunch_iter(self):
        ...
        # string sequence should fail with TypeError
        bunch = G.nbunch_iter("foo")
    ...
    # TODO: Examine if we need this test...
    ...
    #    G = self.K3.copy()
    #    G.add_edge(0, 0)
    #    assert_nodes_equal(sx.nodes_with_selfloops(G), [0])
    #    assert_edges_equal(sx.selfloop_edges(G), [(0, 0)])
    #    assert sx.number_of_selfloops(G) == 1
    #    G.remove_edge(0, 0)
    #    G.add_edge(0, 0)
...
class BaseAttrGraphTester(BaseGraphTester):
    ...
    # NOTE: Not supported (constructor using a graph)
    ...
    #    assert_edges_equal(sx.selfloop_edges(G, data=True),
    #                       [(0, 0, {}), (1, 1, {'weight': 2})])
    #    assert_edges_equal(sx.selfloop_edges(G, data='weight'),
    #                       [(0, 0, None), (1, 1, 2)])
...
class TestGraph(BaseAttrGraphTester):
    ...
    def setup_method(self):
        ...
        self.K3.add_nodes_from(self.k3nodes)
        self.K3.add_edges_from(self.k3edges)
        assert(list(self.K3.nodes) == self.k3nodes)
        assert(list(self.K3.edges) == self.k3edges)
        assert(self.K3.number_of_edges() == len(self.k3edges))
    ...
    def test_getitem(self):
        ...
        with pytest.raises(KeyError):
            G.__getitem__(4) # Key should not be present
        with pytest.raises(TypeError):
            G.__getitem__(['A'])
